...
class CommonModel extends  Model
{
    ...
    *定制分页样式
    ...
    public function queryjoin($tableName,$tableName1,$field1,$field2,$where,$field=true,$type="all",$paging=array("ON",5,1)){
        if($paging['0']=="ON"){
            ...
            $count=M($tableName)->where($where)->join($tableName1." ON kl_".$tableName.".".$field1."=".$tableName1.".".$field2)->count();
            if($paging['2']>1){
                $start=($paging['2']-1)*$paging['1'];
            }else{
                $start=0;
            }
            $data=M($tableName)->where($where)->join($tableName1." ON kl_".$tableName.".".$field1."=".$tableName1.".".$field2)->field($field)->limit($start,$paging['1'])->select();
            $pagenumber=ceil($count/$paging['1']);
            return array("data"=>$data,"count"=>$count,"pagenumber"=>$pagenumber,"page"=>$paging['2']);
        }
